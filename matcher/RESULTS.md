...
Seven transcendental-chain hits across five source lines. Two were recovered
by the `nMul` correction below and are marked.
...
HIGH went from 3 sites to 5, and both recoveries are genuine underflow
...
not the one first published here.**

An earlier version of this section attributed it to the memory-carried blind
...
the update has two in-loop users, the phi and that store, so `cleanUses`
...
values are genuinely observed. It currently cannot tell that case from an
...
**Refining the guard was measured and declined for v1.**

Scanning the same corpus with `./run_study.sh rejects` runs explain mode over
the harvested bitcode and prints this census:
...
`./run_study.sh rejects`; re-running it returns 460/23/8/135/294 exactly.
...
**783 hits and clean recall clear the gate. The shape survives real
codebases in recognizable form. The pass prototype proceeds.**

The gate was cleared on 781 hits before the `nMul` correction; the corrected
783 does not move the verdict.
...
speed. The rescue-worthy subset is 5 HIGH findings on 4 source lines, and
includes the softmax-denominator idiom named in the intent. The pass needs
the profitability/range analysis in front of the rewrite. Diagnostic-first
(flag the 5, not the 783) is the likely shipping shape.

...
**Success criteria.** (1) The milestone above passes. (2) ~~The propagated result is *more* accurate than the linear re-conversion, not merely equal.~~ **Refuted 2026-08-16, and amended.** Measured against a long-double reference on benign inputs, maximum relative error is 2.3e-16 for the pure linear path, 1.6e-15 for the linear re-conversion, and 2.4e-15 for the propagated form — propagation is the *least* accurate of the three where all three work. The mechanism is the one that broke both accumulator bounds: the subtract `t - L` carries absolute rounding `u·|t - L|`, which is relative error in the result, and `|t - L| ≈ 8` at these inputs. The amended criterion: propagation must be no worse than ~10x the linear re-conversion at benign magnitudes, and must return a correct finite result in the regime where the linear path returns NaN. It buys range, not precision, and the earlier wording claimed the wrong thing. (3) The legality grant is stated per rewrite and honored: no propagation fires without it.
